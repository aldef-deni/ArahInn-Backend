...
class BookingController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            ActivityLogService::log($request->user()->id, 'CREATE_BOOKING', 'booking', $result['booking']->id, $request);
    ...
    public function show(Request $request, string $id)
    {
        ...
        if (!in_array($user->getRoleNames()->first(), $adminRoles) && $booking->user_id !== $user->id) {
    ...
    public function cancel(Request $request, string $id)
    {
        ...
        if (!in_array($user->getRoleNames()->first(), ['superadmin', 'admin']) && $booking->user_id !== $user->id) {
            return response()->json(['success' => false, 'message' => 'Akses ditolak.'], 403);
        }
